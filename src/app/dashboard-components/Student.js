    ...
    let handleApprove = () => {
        ...
        .then(res => {
            if (res.status == 404 || res.status == 500) {
                setLoading(false);
                alert('No se ha podido guardar los datos. Por favor, inténtelo más tarde.');
                return
            }
            return res.json()
        })
        .then( ({ success, usererror, dberror, emailerror, sendmail }) => {
            if (success) {
                let item = document.getElementById(value.identification.id);
                item.style.display = 'none';
                setSnack(true, {message: 'Usuario Activado Exitosamente!', severity: 'success'})
                setTimeout(() => {
                    setSnack(false)
                }, 6500);
                return
            ...
            if ( sendmail ) {
                alert(`El usuario ha sido activado, pero no ha sido posible enviar el correo de confirmación. Por favor, envíelo con la copia del contrato a: ${person.email} `);
            }
        })
        .catch( e => {
            setLoading(false);
                ...
                loading ? <div style={{height: '100%', display: 'flex', justifyContent: 'center'}}><CircularProgress /></div> : (
                    ...
                    >
                        <ListItemButton role={undefined}  sx={{display: 'grid', gridTemplateColumns: '50% 1fr auto', overflowX: 'scroll'}}>
                            ...
                            <span>
                                <Button variant="outlined" sx={{ mr: 1}} color='success' onClick={handleApprove}>
                                    <CheckIcon />
                                </Button>
